...
import { inventoryNFT } from "../../utils/inventory";
    ...
    <div className="w-full flex-col center bg-primary">
      <div className="w-[90%] my-28 text-white relative bg-primary">
        ...
        <div className="between flex-wrap gap-3">
          {inventoryNFT.map((inventory, index) => (
            <div key={index} className="w-[24%] p-3 bg-walletGradient rounded-3xl">
              <div className="relative">
                <img src={inventory.image} alt="inventory" className="rounded-[20px]" />
              ...
              <div className="mt-6">
                <div className="flex gap-3">
                  <div className="w-[50px]">
                    <img src={inventory.profileImage} alt="profile" />
                  </div>
                  <div>
                    <h3 className="text-white text-md oxanium tracking-wide">{inventory.name}</h3>
                    <div className="flex gap-1 items-center">
                      <p className="lato text-sm tracking-wide">By: {inventory.author}</p>
                      <div className="w-[15px]">
                        <img src={inventory.profileStatus} alt="profile status" />
                      </div>
                    </div>
                  </div>
                </div>
                <div className="mt-10 between">
                  <p className="text-sm">🔥 Currently: {inventory.status}</p>
                  <p className="oxanium text-md">{inventory.price} DUN</p>
                </div>
                <button
